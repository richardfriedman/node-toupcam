 ...
 */
describe ('Camera PullModeTempTint Test', () => {

  let cam = new CameraPrep(false);
  ...
  it( 'Test AutoWhiteBalance TempTint Callback', function(done){
    let ttCB = toupcam.callback.TempTint( function( temp, tint, context ){
      expect( temp ).to.be.at.least(toupcam.define.TEMP_MIN);
      expect( tint ).to.be.at.least(toupcam.define.TINT_MIN);
      done();
    });
    result = toupcam.lib.Toupcam_AwbOnePush( cam.handle, ttCB, toupcam.Null );
    expectResult(result);
  });
